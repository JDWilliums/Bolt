...
export default function ImageOptCartView() {
  const [cartProducts, setCartProducts] = useState<CartProduct[]>([]);
  const [loading, setLoading] = useState(true);
  const [updating, setUpdating] = useState<number | null>(null);
  const [checkingOut, setCheckingOut] = useState(false);

  // ANTI-PATTERN: SEQUENTIAL FETCH WATERFALL (kept from control)
  useEffect(() => {
  ...
  return (
    <CartContainer>
      ...
      {cartProducts.map((item, index) => {
        ...
        const blurDataURL = blurMap[item.product.imageUrl];
        ...
        return (
          ...
          >
            {/* OPTIMISED: next/image with blur placeholder */}
            <ItemImage>
              {index === 0 ? (
                <Image
