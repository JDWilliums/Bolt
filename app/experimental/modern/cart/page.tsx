...
// ──────────────────────────────────────────────────────────────────
// OPTIMISATION: SERVER COMPONENT CART
// The cart is read from cookies on the server, and all product data
// is fetched in a single batched DB query (vs the control group's
// sequential fetch waterfall). The HTML is streamed to the client
// with full product data already populated — zero client-side JS
// needed for the initial render.
// ──────────────────────────────────────────────────────────────────
export default async function ModernCartPage() {
  return (
...
async function CartContent() {
  ...
  // Single batched DB query for all cart products
  ...
  // Build the full cart data with product details
  ...
  return (
    <div>
      {/* ─── CART ITEMS — Client component with useOptimistic ── */}
      <CartItems
...
function CartSkeleton() {
  return (
    <div>
      {/* Cart item skeleton — single item to avoid CLS from count mismatch */}
      ...
      {/* Order summary skeleton — matches real summary dimensions */}
      <div className="mt-8 p-6 bg-neutral-50 rounded-2xl border border-neutral-200">
